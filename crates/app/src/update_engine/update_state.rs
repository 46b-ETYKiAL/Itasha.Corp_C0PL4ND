...
//!
//! [`super::rollback_guard`] already keeps a strictly-monotonic VERSION floor.
//! The signed manifest carries a parallel `release_index`
...
//! signed-but-older replay window at check time the same way `rollback_guard`
//! closes it at apply time. The two are deliberately redundant defenses; this
//! one is the manifest-native ordinal.
//!
//! ## Persistence — sibling-of-exe, mirroring `rollback_guard`
//!
//! The high-water index is a single integer line in
//! `<exe-dir>/.c0pl4nd-release-index`, written next to the running executable —
//! the SAME directory `rollback_guard`'s `.c0pl4nd-installed-version` record and
//! the keep-one-prior `.c0pl4nd-bak` backup already use. This keeps the
//! updater's persisted state co-located, needs NO new dependency (plain
//! `std::fs` text I/O), and inherits the install dir's permissions. C0PL4ND has
//! no `dirs`/`ProjectDirs` data-dir helper, and the exe-sibling location is the
//! established convention for updater state, so it is reused here.
//!
...
//! record (and, redundantly, from `rollback_guard`'s version floor) regardless.
//! Writes are MONOTONIC — the record is only ever advanced upward.

use std::path::{Path, PathBuf};

/// File name of the persisted "highest release_index ever applied" record,
/// stored next to the running executable.
const RELEASE_INDEX_FILE: &str = ".c0pl4nd-release-index";
...
mod tests {
    ...
    fn record_round_trips_and_is_monotonic() {
        ...
        record_applied_index(&exe, 4009).unwrap();
        assert_eq!(read_applied_index(&exe), 4009);
